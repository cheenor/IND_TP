      PROGRAM OUTPUT_CLOUD
      IMPLICIT NONE
      INTEGER IFS,IFE
      CHARACTER CHENM*200,FILESTR*20
      CHARACTER DATESTR(3)*30,CASENM*30
      CHARACTER CASENMSTR(3)*20,DIRIN*100,DIROUT*100
      CHARACTER DAYSTR*3,stepstr*3,rgname*10
      INTEGER I,J,K,IFL,IDAY,ISKP,it
      INTEGER DCB(3),DCT(3)
      real PBL1(3),PBL2(3)
      CASENMSTR(3)='MLYR2D0'   ; DATESTR(3)='20100101'
      CASENMSTR(2)='PRD2D0'   ; DATESTR(2)='20100101'
      CASENMSTR(1)='ETP2D0'      ; DATESTR(1)='20100101'
      DCB(3)=10 ! ~0.6
      DCT(3)=24 ! 9.9km
      DCB(2)=10 ! ~0.6
      DCT(2)=24 ! 9.9km
      ...
      DO I =1,3
        ...
        CASENM=CASENMSTR(I)
        rgname=CASENM(1:3)
        if (CASENM(1:3)=='MLY')rgname=CASENM(1:4)
        DO IFL= 1,73
          IFS=IFL
          IFE=IFL
          ISKP=0
          DO J=1,5 ! every file contain five days dataset
            WRITE(DAYSTR,'(I3.3)')IDAY
            do it=1,96
            DIRIN='/chenjh2/Models/CRM/ERA/Year/'//trim(rgname)//'/run'
            if(i==1)DIRIN='/chenjh2/Models/CRM/ERA/Year/'//
     &                trim(rgname)//'/run2'
            DIROUT='../Data/'
            write(stepstr,'(I3.3)') it
            FILESTR="Day"//DAYSTR//stepstr
            CALL OUTPUT(DIRIN,DIROUT,FILESTR,CASENM,
     &           IFS,IFE,ISKP,DCB(I),DCT(I),PBL1(I),PBL2(I))
            ISKP=(J-1)*96+it
            enddo
            IDAY=IDAY+1
          ENDDO
      ...
      END PROGRAM
!#########################################################
      SUBROUTINE OUTPUT(DIRIN,DIROUT,SEASON,CASENM,
     &           IFS,IFE,NSKP,DCB,DCT,PBL1,PBL2)
...
!C     ------------------------------------------------------------------
     ...
     $    ,CWP                  ! CLOUD WATER PATH THRESHOLD (G/M2)
      ...
      DATA QCC,QCE,CWP/1.0E-3,1.0E-4,0.0/
...
!C
     ...
     *          , FILENU*3,FPATH*150,DIROUT*100
      ...
      INTEGER IH,IA,IK
!(((((((((((((((((((((((((((((())))))))))))))))))))))))))))))
      REAL QCR(IMR,KMR),QRR(IMR,KMR),QAR(IMR,KMR)
      REAL QBR(IMR,KMR),RHOR(KMR)
      REAL UR(IMR,KMR),WR(IMR,KMR),THR(IMR,KMR)
      REAL Q_R(IMR,KMR),RHR(IMR,KMR)
      REAL TER(KMR),PSR(KMR),POTFR(KMR),POTER(KMR)
      REAL TMPTC,PBL1,PBL2
      INTEGER IX,IFS,IFE, NSKP, ISKP !
      CHARACTER IDSTR*3,SEASON*20,CHENM*200
      INTEGER DCT,DCB,IN,IF
      ZTP=Z(DCT)
      ZBP=Z(8) !~1,5KM
!      DCB=5  ! ~0.6
!      DCT=24 ! 9.9km
...
!     +  TRIM(DATESTR(IGN))//'\SIMULATION\'
     ...
     +  '_OUTPUT_DH_F90'//TRIM(SEASON)//'.TXT'      ! ASCII
     ...
     +        STATUS='OLD',CONVERT='BIG_ENDIAN')
          ...
          DO IN=1,NSKP
            ...
            READ(IH) 
          ENDDO
          DO 101 IN=1,1 !INN. JUST ONE output TIMESTEP
            IT=IT+1
            READ(IH) QCR,QRR 
            READ(IH) QAR,QBR
            READ(IH) THR,Q_R
            ...
            DO IG =5,5 !NGSS,NG  001 -200
              ...
              DO I = 1,NI
                ...
                !   endif
                !enddo
                IF (IG==5) THEN ! 1-100,101-200,51-150
                  ...
                  DO IS = 1,NS  ! LAYER OF CLOUDS
                    ...
                    !IF (Z(KE).GE.ZTD.AND.Z(KB).LE.ZBD) THEN ! ZTD ZTB
                     IF(KE.GE. DCT .AND. KB .LE. DCB .and. KB>1) THEN
                      ...
                      WZ(KB) = WZ(KB) + 1.
                      DO K = KB,KE
                      QD(K,1,1) = QD(K,1,1) + C(K) ! TOTAL WATER CONTENT
                      print*,'CONVECTION!!!!!!!!!' 
                      QD(K,1,2) = QD(K,1,2) + FRG    ! C(K)/QZD  ! cloud water/mean cloud water
                      print*,QD(K,1,2)
                      QD(K,1,3) = QD(K,1,3) + C(K)/QZD*D(K) !
                      !QD(K,KB,3) = QD(K,KB,3) + C(K)/EZD
                      TD(K,1)   = TD(K,1)   + TC(II,K)
                      WD(K,1)   = WD(K,1)   + 1.
                      ENDDO
                    ENDIF
                  ENDDO
                  ...
                  DO IS = 1,NS
                    ...
                    IF(Z(KB).LE. PBL1 .AND. (Z(KE)-Z(KB)).GE.PBL2 
     +                                    .and. KB>1 ) THEN
                    CZD = 0.
                    QZD = 0.
                     print*,'PBL CONVECTION!!!!!!!!!!!'
                    DO K = KB,KE
                      CZD = CZD + ZD(K)
                      QZD = QZD + ZD(K)*C(K)
                    ENDDO
                    ...
                    WB(KE) = WB(KE) + 1.
                    DO K = KB,KE
                      QP(K,1,1) = QP(K,1,1) + C(K)
                      QP(K,1,2) = QP(K,1,2) + C(K)/QZD
                      QP(K,1,3) = QP(K,1,3) + C(K)/EZD*D(K)
                      !QP(K,KE,3) = QP(K,KE,3) + C(K)/EZD
                      WP(K,1) = WP(K,1) + 1.
                    ENDDO
                    ENDIF
                  ENDDO
                  ...
                  IF (KE-KB.LT.KSC) THEN
                    ...
                    !QZD = QZD/CZD
                    TZD = TZD/CZD
                    QS(KB,1) = QS(KB,1) + QZD/CZD   ! mean cloud water
                    QS(KB,2) = QS(KB,2) + QZD         ! cloud water path base top
                    QS(KB,3) = QS(KB,3) + QZD/EZD*D(KB)
                    !QS(KB,3) = QS(KB,3) + QZD/EZD
                    TS(KB) = TS(KB) + TZD
                    WS(KB) = WS(KB) + 1.
                  ENDIF
                  ENDIF
                  ...
                  IF (LC.GT.1) THEN
                    ...
                    ENDDO
                  ENDIF
                  !GET C-----OUTPUTFREQUENCY ACCORDING TO OVERLAP CELLS
                  IF (LC.GT.0) THEN 
                  CE = CE + FRG
                  IO = MIN(LC,NO+1)
                  WA(IO) = WA(IO) + FRG
  ...
  100   CONTINUE  
       ...
       ! ENDDO
        ...
        CLOSE(OU)
        FPATH =DIROUT(1:LENCHR(DIROUT))//TRIM(CASENM)//
     &    '_ALLCLOUDCELSS_FREQUENCY_F90'//TRIM(SEASON)//'.TXT'      ! ASCII
        OPEN(OU,FILE=FPATH(1:LENCHR(FPATH)) )
        DO IG = 1,NG
          WRITE(OU,'(A,I2,F8.3,A)')
...
!                                         # Z[KB] IS BASE,Z[KE]IS CLOUD TOP
        ...
        CLOSE(OU)
        FPATH =DIROUT(1:LENCHR(DIROUT))//TRIM(CASENM)//
     &  '_OVERLAPCLOUDCELSS_FREQUENCY_F90'//TRIM(SEASON)//'.TXT'      ! ASCII
        OPEN(OU,FILE=FPATH(1:LENCHR(FPATH)) )
        DO LC = 1,NE
          DO IO = 1,NO
            WRITE(OU,'(A,I2,F8.3,A)')
     ...
     $            (Z(KE)*10.),((FB(KB,KE,IO,LC)*100.),KB=1,KM)
        ...
        CLOSE(OU)
        FPATH =DIROUT(1:LENCHR(DIROUT))//TRIM(CASENM)//
     &   '_DEEPCONVECTION_INFO_F90'//TRIM(SEASON)//'.TXT'      ! ASCII
        OPEN(OU,FILE=FPATH(1:LENCHR(FPATH)) )
        WRITE(OU,'(A)')
     ...
     $            (Z(K)*10.),((QD(K,KB,IP)*100.),KB=1,20)
        ...
        CLOSE(OU)
        FPATH =DIROUT(1:LENCHR(DIROUT))//TRIM(CASENM)//
     &       '_PBLDEEPCONVECTION_INFO_F90'//TRIM(SEASON)//'.TXT'      ! ASCII
        OPEN(OU,FILE=FPATH(1:LENCHR(FPATH)) )
        WRITE(OU,'(A)')
     $         '======) CEM PBL CONVECTION ... SAMPLES'
        DO K = KM,1,-1                           ! TOP TO SFC
          WRITE(OU,'(31E12.4)')
     $            (Z(K)*10.), ((WP(K,KE)*.1+.01),KE=1,20)
        ENDDO
        WRITE(OU,'(A,30E12.4)') 'Z+> ',((Z(KE)*10.),KE=1,20)
        DO IP = 1,NP
          WRITE(OU,'(A,I2)')
     $         '======) CEM PBL CONVECTION ... PROFILE ',IP
          DO K = KM,1,-1                           ! TOP TO SFC
            WRITE(OU,'(31E12.4)')
     $            (Z(K)*10.),((QP(K,KE,IP)*100.),KE=1,20)
          ENDDO
          WRITE(OU,'(A,30E12.4)') 'Z+> ',((Z(KE)*10.),KE=1,20)
        ENDDO
        CLOSE(OU)
        FPATH =DIROUT(1:LENCHR(DIROUT))//TRIM(CASENM)//
     &  '_SHALLOWCONVECTION_INFO_F90'//TRIM(SEASON)//'.TXT'      ! ASCII
        OPEN(OU,FILE=FPATH(1:LENCHR(FPATH)) )
        WRITE(OU,'(A)')
