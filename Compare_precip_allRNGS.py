...
wd=[2,2,2,2,2]
ny=2010-1990
simpre=np.zeros(shape=(ny,12),dtype=float)
trmpre=np.zeros(shape=(ny,12),dtype=float)
erapre=np.zeros(shape=(ny,12),dtype=float)
...
for iy in range(0,ny):
    ...
    ndays[1]=28
    nt=96*365 
    if iyy%400==0 or (iyy%4==0 and iyy%100!=0):
        ndays[1]=29
        nt=96*366
    else:
        ndays[1]=28   
    ...
    jd=1
    area='ETP'
    datestr="%4d"%iyy+"%2.2d"%im+"%2.2d"%jd+'_365d'    
#------------------------------------------------------------------------------
    fpath=dirin+yearstr+'_preci_'+area+'2D0.txt'
    iskp=0
